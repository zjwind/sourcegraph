...
func TestGitCommitResolver(t *testing.T) {
	...
	t.Run("Lazy loading", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			name: "author",
			want: toSignatureResolver(stores, &commit.Author, true),
			have: func(r *GitCommitResolver) (interface{}, error) {
				return r.Author(ctx)
			},
		}, {
			name: "committer",
			want: toSignatureResolver(stores, commit.Committer, true),
			have: func(r *GitCommitResolver) (interface{}, error) {
			...
			t.Run(tc.name, func(t *testing.T) {
				repo := NewRepositoryResolver(stores, &types.Repo{Name: "bob-repo"})
				// We pass no commit here to test that it gets lazy loaded via
				// the git.GetCommit mock above.
				r := toGitCommitResolver(repo, stores, "c1", nil)
