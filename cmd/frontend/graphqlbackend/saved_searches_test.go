...
func TestSavedSearches(t *testing.T) {
	...
	savedSearches, err := (&schemaResolver{stores: stores}).SavedSearches(ctx)
	if err != nil {
		t.Fatal(err)
	}
	want := []*savedSearchResolver{{stores, types.SavedSearch{
		ID:          key,
...
func TestCreateSavedSearch(t *testing.T) {
	...
	savedSearches, err := (&schemaResolver{stores: stores}).CreateSavedSearch(ctx, &struct {
		Description string
		Query       string
		NotifyOwner bool
		NotifySlack bool
	...
	_, err = (&schemaResolver{stores: stores}).CreateSavedSearch(ctx, &struct {
		Description string
		Query       string
		NotifyOwner bool
		NotifySlack bool
...
func TestUpdateSavedSearch(t *testing.T) {
	...
	userID := MarshalUserID(key)
	savedSearches, err := (&schemaResolver{stores: stores}).UpdateSavedSearch(ctx, &struct {
		ID          graphql.ID
		Description string
		Query       string
		NotifyOwner bool
	...
	_, err = (&schemaResolver{stores: stores}).UpdateSavedSearch(ctx, &struct {
		ID          graphql.ID
		Description string
		Query       string
		NotifyOwner bool
...
func TestDeleteSavedSearch(t *testing.T) {
	...
	_, err := (&schemaResolver{stores: stores}).DeleteSavedSearch(ctx, &struct {
		ID graphql.ID
